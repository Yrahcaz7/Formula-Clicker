...
function pointButtonGain() {
	var imp = game.improvements[5] + game.improvements[10] + game.improvements[24];
	var a = get_alpha();
	var b = get_beta();
	var g = get_gamma();
	var d = get_delta();
	...
	var z = get_zeta();
	var co = get_constant();
	var g_ex = get_g_exponent();
	var d_ex = get_d_exponent();
	var e_ex = get_e_exponent();
	var mul = get_multiplier();
...
function update() {
	...
	if (!document.getElementById("pointButton")) {
		...
		append.onclick = () => {
			...
			if (game.improvements[15] > 0 && game.wave.points < game.wave.pointMax) {
				var gen = game.wave.pointGen * (improvements[15].effect() + improvements[26].effect());
	...
	if (document.getElementById("varDisplay")) {
		...
		var text = "Your " + alpha + " is " + format(get_alpha());
	...
	for (var index = 0; index < upgrades.length; index++) {
		...
		if (element.unlocked() && game.upgrades[index] < max) {
			...
			for (var iteration = 0; iteration < work; iteration++) {
	...
	for (var index = 0; index < improvements.length; index++) {
		...
		if (document.getElementById("improvement_" + index)) {
			...
			var rows = document.getElementById("improvement_" + index).innerHTML.split("<br>");
	...
	for (var index = 0; index < options.length && index < game.improvements[4]; index++) {
		...
		if (element.type == "color") {
			...
			if (document.getElementById("option_" + index + "_type")) {
				var val = document.getElementById("option_" + index + "_type").value;
			...
			if (!document.getElementById("option_" + index + "_type") && game.tab == "options") {
				var append = document.createElement("select");
				append.id = "option_" + index + "_type";
				append.value = game.options[element.id];
				if (append.value === undefined) append.value = element.default;
				document.getElementById("options").appendChild(append);
				for (var num = 0; num < element.list.length; num++) {
	...
	if (game.tab == "waves") {
		...
		if (document.getElementById("wave_graph") && sinwaves.length) {
			...
			if (game.wave.min < game.wave.max) {
				for (var iteration = 0; iteration <= 302; iteration++) {
	...
	for (var index = 0; index < wave_upgrades.length; index++) {
	...
	for (var index = 0; index < infinity_milestones.length; index++) {
		...
		if (game.tab != "infinity" || game.infinity.milestones[index - 1] === false) {
			...
			boolean = true;
			for (var iteration = 0; iteration < element.merge.length; iteration++) {
	...
	if (game.tab == "???") {
		...
		var text = "<div class=v0><span style='font-size:calc(var(--text-size)*2)'>", count = +game.infinity.points;
		for (var iteration = 0; iteration < poem.length; iteration++) {
...
const loop = setInterval(() => {
	if (game.unlocks.includes("w")) {
		...
		// wave point gain
		var gen = findNumber(Math.abs((sinwaves[game.wave.frame+151] / 100) - 1), min, max);
		gen *= waveMult();
