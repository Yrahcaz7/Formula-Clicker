...
function pointButtonGain() {
	...
	if (game.improvements[5] > 2) return (co * a * b * g * d) * ((1.1 * g) ** (ex + (d ** 0.5)));
...
function update() {
	...
	if (game.tab == "main" || game.tab == "improvements") {
		...
		if (document.getElementById("varDisplay")) {
			...
			if (game.improvements[5] > 2) text = "Your point gain is "+format(get_constant())+alpha+beta+gamma+delta+"(1.10"+gamma+" ^ ("+format(get_exponent())+" + ("+delta+" ^ 0.50)))<br><br>"+text+"<br>Your "+beta+" is "+format(get_beta())+"<br>Your "+gamma+" is "+format(get_gamma())+"<br>Your "+delta+" is "+format(get_delta());
