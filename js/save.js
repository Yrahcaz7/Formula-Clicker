...
function getProxy() {
	...
	get = get.replace(/e\+/g, "e").replace(new RegExp(MAX, "g"), "null").replace(/null/g, "!N");
...
function normalizeSave(save = localStorage.getItem(ID)) {
	...
	// fix numbers
	save = save.replace(/null|!N/g, MAX);
	// fix decimals
	...
	// fix finish time
	if (!result.finishTime || result.infinity.stage < MAX) result.finishTime = -1;
	// update version
