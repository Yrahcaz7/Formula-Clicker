const improvements = [{
	...
	max: 1,
	unlocked() { return game.upgrades[3] > 0 && game.improvements[0] > 0 },
}, {
	title: "ADVANCEMENT",
	...
	max: 4,
	unlocked() { return game.upgrades[5] > 0 && game.improvements[1] },
}, {
	title: "AUTOMATION",
	...
	max: 1,
	unlocked() { return game.improvements[2] > 3 },
}, {
	...
	desc() { return (game.improvements[15] > 0 ? "increases wave point gain from the point button by " + format(5, true, false, true) + "% of your wave points per second" : "unlocks getting wave point gain from the point button (" + format(5, true, false, true) + "% of your wave points per second)") },
	...
	max: 1,
	unlocked() { return game.improvements[14] > 4 },
}, {
	...
	cost() { return new Decimal(1e162) },
	max: 1,
	unlocked() { return game.improvements[17] > 4 },
}, {
	...
	max: 4,
	unlocked() { return game.improvements[21] && game.infinity.stage > 1 },
}, {
	title: "EVEN MORE",
	...
	desc() { return "improves the point gain formula" },
	cost() { return new Decimal("1e545") },
	max: 1,
	unlocked() { return game.improvements[23] > 9 },
}, {
	...
	cost() { return new Decimal("1e1250") },
	max: 1,
}];

// sets default unlock condition
for (let index = 0; index < improvements.length; index++) {
	if (improvements[index].unlocked === undefined) improvements[index].unlocked = () => { return game.improvements[index - 1] > 0 };
};
